...
/// Initialized by `voice::init()` from the app's OS data directory so
/// `model_dir()` is available to every command without threading an
/// `AppHandle` through every call signature.
static MODEL_DIR: OnceLock<PathBuf> = OnceLock::new();

/// Call once during app setup with a real `AppHandle` so the model directory
/// resolves to the platform-correct app-data path.
pub fn init(app: &tauri::AppHandle) {
    if let Ok(dir) = app.path().app_data_dir() {
        let _ = MODEL_DIR.set(dir.join("whisper-models"));
    }
}
...
fn model_dir() -> PathBuf {
    ...
    MODEL_DIR.get().cloned().unwrap_or_else(|| PathBuf::from("whisper-models"))
}
...
#[tauri::command]
pub async fn voice_model_status() -> Result<VoiceModelStatus, String> {
    Ok(VoiceModelStatus {
...
/// Frontend IPC: starts a new (never-locked) recording. Idempotent only in
/// the error sense — a second call while already recording returns an error
/// rather than silently doing nothing.
#[tauri::command]
pub async fn start_dictation<R: tauri::Runtime>(app: tauri::AppHandle<R>) -> Result<(), String> {
    start_recording_internal(app, false).await
}

/// Frontend IPC: stops the active recording and begins transcription.
/// Errors if nothing is currently recording.
#[tauri::command]
pub async fn stop_dictation<R: tauri::Runtime>(app: tauri::AppHandle<R>) -> Result<(), String> {
    stop_recording_and_transcribe_internal(app).await
}
...
mod tests {
    ...
    fn is_meaningful_transcription_accepts_real_text_with_low_no_speech_probability() {
        assert!(super::is_meaningful_transcription(&transcription("apply to the jobs", &[0.05])));
    ...
    fn is_meaningful_transcription_accepts_text_with_no_segments() {
        assert!(super::is_meaningful_transcription(&transcription("apply to the jobs", &[])));
    }

    #[test]
    fn is_meaningful_transcription_rejects_high_no_speech_probability() {
        assert!(!super::is_meaningful_transcription(&transcription("Thank you.", &[0.9])));
    }

    #[test]
    fn is_meaningful_transcription_accepts_mixed_segments() {
        assert!(super::is_meaningful_transcription(&transcription("apply to the jobs", &[0.9, 0.05])));
    }

    #[test]
    fn is_meaningful_transcription_rejects_bracketed_annotations() {
        for annotation in ["[BLANK_AUDIO]", "[SILENCE]", "[no audio]", "(silence)", "[music]"] {
            assert!(
                ...
                "expected {annotation:?} to be rejected"
            );
        }
    }

    #[test]
    fn is_meaningful_transcription_accepts_text_with_brackets() {
        assert!(super::is_meaningful_transcription(&transcription("the file is [draft].txt", &[0.05])));
    }

    #[test]
    fn is_bracketed_annotation_matches_whisper_style() {
    ...
    #[test]
    fn is_bracketed_annotation_rejects_mismatched() {
